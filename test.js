...
for(const id of ['rating','lightningBtn','bossBtn','practiceBtn','friendBtn','impossibleBtn','feed','achievements','proBtn','upgradeBtn','manageBtn','share','sharePreview','challengeBanner','acceptChallengeBtn','companyCard','companyInterestBtn','companyModal','saveCompanyInterest','copyCompanyLinkedIn'])assert(new RegExp(`id=["']${id}["']`).test(html),`missing UI hook: ${id}`);
...
for(const token of ['beatai.games','Turn AI literacy into a team sport','3 packs · 15 new AI-generated questions each','SHARE YOUR SCORE','Beat AI coolbot'])assert(html.includes(token),`missing page token: ${token}`);
